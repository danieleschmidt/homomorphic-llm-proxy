...
async fn test_complete_fhe_workflow() {
    ...
    // Test validation
    assert!(engine.validate_ciphertext(&ciphertext).expect("Validation failed"));
...
async fn test_fhe_params_edge_cases() {
    ...
    assert!(!report.is_valid);
    assert!(report.errors.iter().any(|e| e.field == "poly_modulus_degree"));
    ...
    assert!(!report.is_valid);
    assert!(report.errors.iter().any(|e| e.field == "coeff_modulus_bits"));
}