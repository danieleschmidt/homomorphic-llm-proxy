...
async fn test_fhe_engine_basic_operations() {
    ...
    let ciphertext = fhe_engine.encrypt_text(client_id, plaintext).expect("Failed to encrypt");
...
fn test_fhe_params_serialization() {
    ...
    let deserialized: FheParams = serde_json::from_str(&serialized).expect("Failed to deserialize params");
