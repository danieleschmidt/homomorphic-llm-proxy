...
fn bench_encryption(c: &mut Criterion) {
    ...
    c.bench_function("fhe_encrypt_text", |b| {
        b.iter(|| {
            engine
                .encrypt_text(black_box(client_id), black_box(test_data))
                .unwrap()
        })
    });
}
...
fn bench_decryption(c: &mut Criterion) {
    ...
    c.bench_function("fhe_decrypt_text", |b| {
        b.iter(|| {
            engine
                .decrypt_text(black_box(client_id), black_box(&ciphertext))
                .unwrap()
        })
    });
}
...
fn bench_homomorphic_ops(c: &mut Criterion) {
    ...
    c.bench_function("fhe_concatenate", |b| {
        b.iter(|| {
            engine
                .concatenate_encrypted(black_box(&ciphertext_a), black_box(&ciphertext_b))
                .unwrap()
        })
    });
}
